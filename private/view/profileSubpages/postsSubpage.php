<div class="allposts">
    <form action="profile.php?action=newpost&id=<?php echo $info['_id']->{'$id'};?>" method="post">
        <input type="text" class= "writepost" placeholder="Share<?php             
            if ($info['_id']->{'$id'} != $_SESSION['id']) {
                echo ' with '.$info['firstname'];
            }
            ?>" name="newpost">
        <input type="text" style="visibility:hidden;width:0;height:0;" name="postTo" value="<?php echo $info['_id']->{'$id'};?>">
        <input type="submit" class="writepostsubmit" value="Post">
    </form>
    <?php
        $i = 0;
                date_default_timezone_set('EST');
                foreach ($profposts as $post) {
                    ?>
